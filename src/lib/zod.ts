...
export const createReservationSchema = object({
  ...
  roomId: number({ required_error: "Sala konferencyjna jest wymagana" }).int().nonnegative(),
  categoryId: number({ required_error: "Kategoria jest wymagana" }).int().nonnegative(),
  userId: number({ required_error: "Użytkownik jest wymagany" }).int().nonnegative(),
})